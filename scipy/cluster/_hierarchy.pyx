...
cpdef void calculate_cluster_sizes(float[:, :] Z, float[:] cs, int n):
    """
...
def cluster_dist(float[:, :] Z, int[:] T, float cutoff, int n):
    """
    ...
    n : int
        The number of observations.
    """
    cdef float[:] max_dists = np.ndarray(n, dtype=np.float32)
    get_max_dist_for_each_cluster(Z, max_dists, n)
    cluster_monocrit(Z, max_dists, T, cutoff, n)


def cluster_in(float[:, :] Z, float[:, :] R, int[:] T, float cutoff, int n):
    """
    ...
    n : int
        The number of observations.
    """
    cdef float[:] max_inconsists = np.ndarray(n, dtype=np.float32)
    get_max_Rfield_for_each_cluster(Z, R, max_inconsists, n, 3)
    cluster_monocrit(Z, max_inconsists, T, cutoff, n)


def cluster_maxclust_dist(float[:, :] Z, int[:] T, int n, int mc):
    """
...
cpdef void cluster_maxclust_monocrit(float[:, :] Z, float[:] MC, int[:] T,
                                     int n, int max_nc):
    """
...
cpdef void cluster_monocrit(float[:, :] Z, float[:] MC, int[:] T,
                            float cutoff, int n):
    """
...
def cophenetic_distances(float[:, :] Z, float[:] d, int n):
    """
...
cpdef void get_max_Rfield_for_each_cluster(float[:, :] Z, float[:, :] R,
                                           float[:] max_rfs, int n, int rf):
    """
...
cpdef get_max_dist_for_each_cluster(float[:, :] Z, float[:] MD, int n):
    """
    ...
    """
    cdef int k, i_lc, i_rc, root
    cdef float max_dist, max_l, max_r
    cdef int[:] curr_node = np.ndarray(n, dtype=np.intc)
...
def inconsistent(float[:, :] Z, float[:, :] R, int n, int d):
    """
    ...
    cdef int[:] curr_node = np.ndarray(n, dtype=np.intc)
    cdef float level_sum, level_std_sum, level_std, dist
...
def leaders(float[:, :] Z, int[:] T, int[:] L, int[:] M, int nc, int n):
    """
...
def linkage(float[:] dists, np.npy_int64 n, int method):
    """
    ...
    """
    Z_arr = np.empty((n - 1, 4), dtype=np.float32)
    cdef float[:, :] Z = Z_arr

    cdef int i, j, k, x, y, nx, ny, ni, id_x, id_y, id_i
    cdef np.npy_int64 i_start
    cdef float current_min
    # inter-cluster dists
    cdef float[:] D = np.ndarray(n * (n - 1) / 2, dtype=np.float32)
...
cdef Pair find_min_dist(int n, float[:] D, int[:] size, int x):
    cdef float current_min = NPY_INFINITYF
    cdef int y = -1
    cdef int i
    cdef float dist
...
def fast_linkage(float[:] dists, int n, int method):
    """Perform hierarchy clustering.
    ...
    """
    cdef float[:, :] Z = np.empty((n - 1, 4), dtype=np.float32)

    cdef float[:] D = dists.copy()  # Distances between clusters.
    ...
    cdef float[:] min_dist = np.empty(n - 1, dtype=np.float32)
...
def nn_chain(float[:] dists, int n, int method):
    ...
    cdef float dist, current_min

    for k in range(n - 1):
        if k % ((n-1)//100) == 0:
            print('nn_chain: Progress %d/%d'%(k, (n-1)))
        if chain_length == 0:
            chain_length = 1
            for i in range(n):
                if size[i] > 0:
...
def mst_single_linkage(float[:] dists, int n):
    ...
    cdef float[:] D = np.empty(n, dtype=np.float32)
    D[:] = NPY_INFINITYF
...
def prelist(float[:, :] Z, int[:] members, int n):
    """
